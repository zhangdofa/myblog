...
@section('content')
    <div class="right">
        <div class="content">
   <form action="{{ url('index/publish') }}" method="post" accept-charset="utf-8"> 
    @csrf
    <div class="substance1">
      <h3>发布文章</h3>
                <div class="describe">
      <div class="title3">请输入标题:<input type="text" name="title" value="{{old('title')}}"></div>
      <div class="error">{{ $errors->first('title') }}</div>
      <div class="title3">请输入描述:<input type="text" name="description" value="{{old('description')}}"></div>
      <div class="error">{{ $errors->first('description') }}</div>
      <textarea type="text" name="content" id="EditorId" placeholder="请输入内容">{{old('content')}}</textarea> 
